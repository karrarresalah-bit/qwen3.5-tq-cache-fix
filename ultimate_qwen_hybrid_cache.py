...
class UltimateHybridCache:
    ...
    def memory_stats(self):
        """Calculates VRAM footprint for BOTH Attention and DeltaNet caches."""
        compressed_bytes = 0
        residual_bytes   = 0
        sparse_bytes     = 0

        # 1. Attention Layers (TurboQuant)
        for store in self._compressed_attn.values():
            if store["k_idx"] is not None:
        ...
        # 2. DeltaNet Layers (Sparse Pruning)
        for k_sparse, v_sparse in self._compressed_delta.values():
            if k_sparse.is_sparse:
                # Sparse tensors store non-zero values + their coordinates
                sparse_bytes += k_sparse._values().numel() * 2  # fp16 values
                sparse_bytes += k_sparse._indices().numel() * 8 # int64 indices
                sparse_bytes += v_sparse._values().numel() * 2
                sparse_bytes += v_sparse._indices().numel() * 8
            else:
                sparse_bytes += k_sparse.numel() * 2
                sparse_bytes += v_sparse.numel() * 2

        total_bytes = compressed_bytes + residual_bytes + sparse_bytes
        return {
            "attn_compressed_kb": compressed_bytes / 1024,
            "attn_residual_kb":   residual_bytes / 1024,
            "delta_sparse_kb":    sparse_bytes / 1024,
